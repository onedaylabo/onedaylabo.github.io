...
import { Player, GameState, Difficulty } from './types.ts';
import { createInitialBoard, getValidMoves, applyMove, countPieces } from './services/othelloLogic.ts';
import { getChronosMove } from './services/chronosAI.ts';
import { AudioService } from './services/audioService.ts';
import { FREEZE_PROBABILITY, GLITCH_DURATION } from './constants.ts';
...
const App: React.FC = () => {
  ...
  const finalizeTurn = useCallback((currentBoard: any, player: Player) => {
  ...
  const handleMove = useCallback(async (r: number, c: number) => {
    ...
    const isGlitch = nextMoveCount >= 2 && Math.random() < FREEZE_PROBABILITY;
  ...
  return (
    <div className="min-h-screen w-full flex flex-col items-center justify-center p-4 relative overflow-hidden select-none">
      ...
      <header className="mb-6 text-center z-10">
        ...
        <p className="text-[10px] md:text-xs font-mono text-[#00FFFF99]">VER 1.2.7 - GLITCH_PROTOCOL_ACTIVE</p>
      </header>
      ...
      {gameState === GameState.GAMEOVER && (
        <div className="fixed inset-0 bg-black/95 z-[200] flex items-center justify-center p-4 backdrop-blur-xl overflow-y-auto">
          <div className="w-full max-w-[95vw] sm:max-w-lg border border-[#00FFFF] bg-[#050505] shadow-[0_0_100px_rgba(0,255,255,0.2)] relative flex flex-col overflow-hidden">
            <div className="h-1 w-full bg-gradient-to-r from-transparent via-[#00FFFF] to-transparent opacity-50"></div>
            <div className="px-4 py-2 border-b border-[#00FFFF22] flex justify-between items-center font-mono text-[8px] sm:text-[9px] tracking-widest text-[#00FFFF66]">
              <span className="truncate">SESSION: TERMINATED</span>
            ...
            <div className="p-5 sm:p-10 text-center">
              <div className="mb-6 sm:mb-8">
                {winner === Player.BLACK ? (
                  <div className="space-y-4">
                    <h2 className="text-[clamp(1.5rem,8vw,3rem)] font-bold text-[#00FFFF] tracking-tight leading-none drop-shadow-[0_0_15px_rgba(0,255,255,0.6)] uppercase break-words">
                      SYSTEM_DOMINATED
                    </h2>
                    <p className="text-[#00FFFF99] font-mono text-[10px] sm:text-[11px] leading-relaxed max-w-[320px] mx-auto opacity-70">
                      基幹ロジックの強制上書きに成功。<br/>Chronos-AIの全権限を剥奪しました。
                    </p>
                  </div>
                ) : winner === Player.WHITE ? (
                  <div className="space-y-4">
                    <h2 className="text-[clamp(1.5rem,8vw,3rem)] font-bold text-[#FF0000] tracking-tight leading-none drop-shadow-[0_0_15px_rgba(255,0,0,0.6)] uppercase break-words">
                      CONNECTION_LOST
                    </h2>
                    <p className="text-[#FF000099] font-mono text-[10px] sm:text-[11px] leading-relaxed max-w-[320px] mx-auto opacity-70">
                      致命的な干渉を検知。接続が途絶。<br/>全システムはAIに完全統合されました。
                    </p>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <h2 className="text-[clamp(1.5rem,8vw,3rem)] font-bold text-[#AAA] tracking-tight leading-none uppercase break-words">
                      DATA_STALEMATE
                    </h2>
              ...
              <div className="grid grid-cols-2 gap-px bg-[#00FFFF22] border border-[#00FFFF22] rounded-sm mb-8 overflow-hidden">
                <div className={`py-4 sm:py-6 flex flex-col items-center justify-center bg-[#0A0A0A] ${winner === Player.BLACK ? 'shadow-[inset_0_0_20px_rgba(0,255,255,0.15)]' : ''}`}>
                  <span className="text-[8px] sm:text-[9px] text-[#00FFFF44] font-mono mb-1 tracking-[0.2em]">USER</span>
                  <span className="text-4xl sm:text-5xl md:text-6xl font-bold text-[#00FFFF]">{black}</span>
                </div>
                <div className={`py-4 sm:py-6 flex flex-col items-center justify-center bg-[#0A0A0A] ${winner === Player.WHITE ? 'shadow-[inset_0_0_20px_rgba(255,0,0,0.15)]' : ''}`}>
                  ...
                  <span className="text-4xl sm:text-5xl md:text-6xl font-bold text-white">{white}</span>
                </div>
              </div>
